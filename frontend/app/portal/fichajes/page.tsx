...
type Row = { id: string; clocked_at: string; milestone?: { type: string } };
type Day = { date: string; label: string; first: string | null; last: string | null; hours: number; incomplete: boolean; empty: boolean };
...
export default function MisFichajesPage() {
  ...
  const week = useMemo<Day[]>(() => {
    ...
    const byDate = new Map<string, { min: number; type: string }[]>();
    for (const r of rows) {
      const d = new Date(r.clocked_at);
      const key = d.toISOString().slice(0, 10);
      if (!byDate.has(key)) byDate.set(key, []);
      byDate.get(key)!.push({ min: d.getHours() * 60 + d.getMinutes(), type: r.milestone?.type ?? "" });
    }
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        last: last !== null ? fmt(last) : null, hours: worked / 60, incomplete, empty: items.length === 0,
      };
    });
  }, [rows]);
        ...
        <Card className="overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-line bg-canvas text-left text-xs uppercase tracking-wide text-ink-soft">
              <tr><th className="px-5 py-3 font-medium">Día</th><th className="px-5 py-3 font-medium">Entrada</th><th className="px-5 py-3 font-medium">Salida</th><th className="px-5 py-3 text-right font-medium">Horas</th></tr>
            </thead>
            ...
            <tfoot>
              <tr className="border-t border-line bg-canvas font-semibold">
                <td className="px-5 py-3" colSpan={3}>Total semana</td>
